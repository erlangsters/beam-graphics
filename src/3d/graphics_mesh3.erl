...
%%
-module(graphics_mesh3).
-moduledoc """
3D Mesh

A 3D mesh is a collection of 3D vertices that is typically used for rendering.

A 3D mesh is an opaque object that wraps GPU vertex data. It is created with
the `with_vertices` functions and disposed with the `destroy/1` function.
Copying the term does not copy the GPU buffer.

Each 3D vertex is a position, a color, and UV texture coordinates (see
`graphics:vertex3()`). Meshes are rendered on a frame or a surface with a
primitive type and an optional texture. The primitive type and the texture are
not part of the mesh; they are arguments of `graphics_frame:draw_mesh3/4`,
`graphics_surface:draw_mesh3/4`, and of `graphics_shape3`.

```erlang
{ok, Mesh} = graphics_mesh3:with_vertices([
    {{100.0, 100.0, 0.0}, ?COLOR_RED, 0.0, 0.0},
    {{700.0, 100.0, 0.0}, ?COLOR_GREEN, 0.0, 0.0},
    {{700.0, 500.0, 0.0}, ?COLOR_BLUE, 0.0, 0.0}
]).
ok = graphics_surface:draw_mesh3(Surface, Mesh, triangles, 3).
```

Together with 2D meshes, meshes are the only means for rendering. Anything
...
Beware that a well-formed 3D vertex always contains floats, not integers.

**OpenGL Internals**

A 3D mesh wraps an OpenGL buffer object. Use `gl_object/1` to retrieve the
buffer id.
""".
...
% Vertex stride in bytes: 3 position + 4 color + 2 UV floats.
...
""".
-opaque object() :: {
    ResourceId :: {mesh3, gl:buffer()},
    VertexCount :: non_neg_integer(),
    UsageHint :: usage_hint(),
    LocalVertices :: undefined | [graphics:vertex3()]
}.

-doc """
A 3D mesh from a list of vertices.

It constructs a 3D mesh from the given 3D vertices. The usage hint is `static`
and no local copy is kept.

It's equivalent to `with_vertices(Vertices, static)`.
""".
-spec with_vertices([graphics:vertex3()]) -> {ok, object()} | out_of_memory.
with_vertices(Vertices) ->
    with_vertices(Vertices, ?DEFAULT_USAGE_HINT).

-doc """
A 3D mesh from a list of vertices and a usage hint.

It constructs a 3D mesh from the given 3D vertices with the given usage hint.
No local copy is kept.

It's equivalent to `with_vertices(Vertices, UsageHint, no_copy)`.
""".
-spec with_vertices([graphics:vertex3()], usage_hint()) ->
...
-doc """
A 3D mesh from a list of vertices, a usage hint, and a copy policy.
...
the buffer.
""".
-spec with_vertices([graphics:vertex3()], usage_hint(), keep_copy()) ->
    {ok, object()} | out_of_memory
.
...
It releases the GPU buffer of the 3D mesh. Using the mesh after it is destroyed
...
-doc """
Set the vertices of a 3D mesh.

It replaces the vertices of the 3D mesh. The usage hint and the copy policy of
the mesh are preserved. The OpenGL buffer id is unchanged.

It's equivalent to `set_vertices(Mesh, Vertices, usage_hint(Mesh))`.
""".
-spec set_vertices(object(), [graphics:vertex3()]) ->
    {ok, object()} | out_of_memory
.
...
-doc """
Set the vertices of a 3D mesh with a usage hint.

It replaces the vertices of the 3D mesh and sets the usage hint. The copy
policy of the mesh is preserved. The OpenGL buffer id is unchanged.

The list may be empty. It returns `out_of_memory` when the GPU cannot allocate
the new data store.
""".
-spec set_vertices(object(), [graphics:vertex3()], usage_hint()) ->
    {ok, object()} | out_of_memory
.
set_vertices(
...
-doc """
The number of vertices of a 3D mesh.

It returns the number of vertices currently stored in the 3D mesh.
""".
...
-doc """
The locally cached vertices of a 3D mesh.
...
""".
-spec local_vertices(object()) -> undefined | [graphics:vertex3()].
...
-doc """
A slice of the locally cached vertices of a 3D mesh.
...
```erlang
{ok, Mesh} = graphics_mesh3:with_vertices(Vertices, static, keep_copy),
{2, [Second]} = graphics_mesh3:local_vertices(Mesh, {1, -1}).
```
""".
-spec local_vertices(object(), slice:range()) ->
    undefined | no_range | {slice:offset(), [graphics:vertex3()]}
.
...
-doc """
A slice of the GPU vertices of a 3D mesh.
...
-spec remote_vertices(object(), slice:range()) ->
    no_range | {slice:offset(), [graphics:vertex3()]}
.
...
-doc """
A locally cached vertex of a 3D mesh.
...
```erlang
{ok, Mesh} = graphics_mesh3:with_vertices(Vertices, static, keep_copy),
{1, {{0.0, 0.0, 0.0}, ?COLOR_RED, 0.0, 0.0}} = graphics_mesh3:local_vertex(Mesh, 0),
{3, {{0.0, 1.0, 0.0}, ?COLOR_BLUE, 0.0, 1.0}} = graphics_mesh3:local_vertex(Mesh, -1).
```
...
-spec local_vertex(object(), slice:index()) ->
    undefined | out_of_range | {slice:offset(), graphics:vertex3()}
.
...
-spec remote_vertex(object(), slice:index()) ->
    out_of_range | {slice:offset(), graphics:vertex3()}
.
remote_vertex(
...
-doc """
Update the vertices of a 3D mesh from the start.
...
""".
-spec update_vertices(object(), [graphics:vertex3()]) ->
...
-doc """
Update the vertices of a 3D mesh from an index.
...
""".
-spec update_vertices(object(), [graphics:vertex3()], slice:index()) ->
    {ok, object()} | out_of_range
.
...
""".
-spec update_vertex(object(), graphics:vertex3(), slice:index()) ->
    {ok, object()} | out_of_range
.
...
-doc """
Check whether a 3D mesh keeps a local copy.
...
-doc """
Keep a local copy of a 3D mesh.
...
-doc """
Release the local copy of a 3D mesh.
...
-doc """
The OpenGL buffer of a 3D mesh.

It returns the OpenGL buffer id wrapped by the 3D mesh.
""".
-spec gl_object(object()) -> gl:buffer().
gl_object({{mesh3, Buffer}, _VertexCount, _UsageHint, _LocalVertices}) ->
    Buffer.
