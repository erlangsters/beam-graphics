...
```erlang
{0.0, 0.0, 0.0} = box3:min(Box).
{10.0, 20.0, 30.0} = box3:max(Box).
```
...
```erlang
true = box3:contains({{0.0, 0.0, 0.0}, {10.0, 10.0, 10.0}}, {5.0, 5.0, 5.0}).
true = box3:intersects(
    {{0.0, 0.0, 0.0}, {2.0, 2.0, 2.0}},
    {{1.0, 1.0, 1.0}, {3.0, 3.0, 3.0}}
).
```

To transform a 3D box with a 4x4 matrix, see the `transform3:transform_box/2`
...
from_center_size(Center, Size) ->
    Half = vector3:multiply(vector3:abs(Size), 0.5),
    {vector3:subtract(Center, Half), vector3:add(Center, Half)}.
...
center({Min, Max}) ->
    vector3:multiply(vector3:add(Min, Max), 0.5).
...
intersection({Min1, Max1}, {Min2, Max2}) ->
    Min = vector3:max(Min1, Min2),
    Max = vector3:min(Max1, Max2),
    {MinX, MinY, MinZ} = Min,
    {MaxX, MaxY, MaxZ} = Max,
...
union({Min1, Max1}, {Min2, Max2}) ->
    {vector3:min(Min1, Min2), vector3:max(Max1, Max2)}.
...
translate({Min, Max}, Vector) ->
    {vector3:add(Min, Vector), vector3:add(Max, Vector)}.
...
is_equal_to({Min1, Max1}, {Min2, Max2}) ->
    vector3:is_equal_to(Min1, Min2) andalso vector3:is_equal_to(Max1, Max2).
...
is_equal_to({Min1, Max1}, {Min2, Max2}, Epsilon) ->
    vector3:is_equal_to(Min1, Min2, Epsilon)
        andalso vector3:is_equal_to(Max1, Max2, Epsilon).
