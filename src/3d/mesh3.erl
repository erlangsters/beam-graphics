...
not part of the mesh; they are arguments of `frame:draw_mesh3/4`,
`surface:draw_mesh3/4`, and of `shape3`.
...
```erlang
{ok, Mesh} = mesh3:with_vertices(Vertices, static, keep_copy),
{2, [Second]} = mesh3:local_vertices(Mesh, {1, -1}).
```
""".
...
```erlang
{ok, Mesh} = mesh3:with_vertices(Vertices, static, keep_copy),
{1, {{0.0, 0.0, 0.0}, ?COLOR_RED, 0.0, 0.0}} = mesh3:local_vertex(Mesh, 0),
{3, {{0.0, 1.0, 0.0}, ?COLOR_BLUE, 0.0, 1.0}} = mesh3:local_vertex(Mesh, -1).
```
