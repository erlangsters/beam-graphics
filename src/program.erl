...
custom program that is drawn through `surface` or `frame` should match that
layout. `program` does not enforce it.
        ...
        case Type of
            ...
            matrix3 ->
                ok = gl:program_uniform_matrix(
                    f, GlProgram, Location, matrix3:columns(Value)
                );
            matrix4 ->
                ok = gl:program_uniform_matrix(
                    f, GlProgram, Location, matrix4:columns(Value)
                )
        end,
        ok
    end).
