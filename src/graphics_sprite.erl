...
case. The shape is a `graphics:shape2()` value; see the `graphics_shape2` module.
...
built with `graphics_shape3:with_mesh/4`.
...
height. The rectangle extends in `+X` and `+Y`, matching `graphics_shape2:rectangle/3`.
...
```erlang
{ok, Sprite} = graphics_sprite:from_texture(
    {0.0, 0.0},
    {100.0, 50.0},
    Texture,
    ?COLOR_WHITE
...
from_texture({X, Y}, {Width, Height}, Texture, Color) ->
    ...
    case graphics_mesh2:with_vertices(Vertices) of
        {ok, Mesh} ->
            {ok, graphics_shape2:with_mesh(Mesh, triangle_fan, 4, Texture)};
        out_of_memory ->
            out_of_memory
    end.
