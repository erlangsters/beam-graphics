...
The default program is the stock GPU shader program used by `frame` and
`surface` to draw meshes and shapes.

This module is an internal helper. It is not part of the public graphics API.
A default program is a `program:object()`. It is created with `new/0` and
disposed with `program:destroy/1`. Copying the term does not copy the GPU
program.

```erlang
{ok, Program} = default_program:new(),
...
matrices are embedded with `matrix3:to_matrix4/1` at draw time.

The stock uniforms are `uModel`, `uView`, and `uProjection` (`matrix4`),
...
A default program is a `program:object()`. Use `program:gl_object/1` to
...
.
new() ->
    case program:with_shaders(?VERTEX_SHADER_SRC, ?FRAGMENT_SHADER_SRC) of
        {ok, Program} ->
            ok = program:set_uniform(Program, "uTexture", 0),
            ok = program:set_uniform(Program, "uUseTexture", false),
            {ok, Program};
        Error ->
            Error
    end.
