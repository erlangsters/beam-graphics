...
not part of the mesh; they are arguments of `graphics_frame:draw_mesh2/5`,
`graphics_surface:draw_mesh2/5`, `graphics_frame:draw_mesh3/5`, `graphics_surface:draw_mesh3/5`, and of
`graphics_shape2` / `graphics_shape3`.
...
{2, 1, {1, 1, [Second]}} = graphics_texture:local_image(Texture, {1, -1}, {undefined, 1}).
```
""".
...
```erlang
{ok, Texture} = graphics_texture:with_image(Image, linear, keep_copy),
{1, 1, Color} = graphics_texture:local_pixel(Texture, 0, 0),
{Width, Height, ColorLast} = graphics_texture:local_pixel(Texture, -1, -1).
```
