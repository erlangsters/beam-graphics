...
```erlang
{0.0, 0.0} = graphics_camera2:center(Camera).
...
`1.0`. Rotation is counter-clockwise in the world, matching `graphics_vector2:rotate/2`.
Zoom greater than `1.0` makes objects appear larger.

```erlang
{{10.0, 20.0}, 0.0, 1.0} = graphics_camera2:from_center({10.0, 20.0}).
```

The view matrix of a 2D camera is constructed with the `view_matrix/1`
function. Combined with a 2D view (see `graphics_view2`), it maps world space to clip
space.
...
view_matrix({Center, Rotation, Zoom}) ->
    graphics_matrix3:multiply(
        graphics_transform2:scale({Zoom, Zoom}),
        graphics_matrix3:multiply(
            graphics_transform2:rotation(-Rotation),
            graphics_transform2:translation(graphics_vector2:negate(Center))
        )
    ).
...
translate({Center, Rotation, Zoom}, Vector) ->
    {graphics_vector2:add(Center, Vector), Rotation, Zoom}.
...
is_equal_to({Center1, Rotation1, Zoom1}, {Center2, Rotation2, Zoom2}) ->
    graphics_vector2:is_equal_to(Center1, Center2)
...
is_equal_to({Center1, Rotation1, Zoom1}, {Center2, Rotation2, Zoom2}, Epsilon) ->
    graphics_vector2:is_equal_to(Center1, Center2, Epsilon)
