...
not part of the mesh; they are arguments of `graphics_frame:draw_mesh2/4`,
`graphics_surface:draw_mesh2/4`, and of `graphics_shape2`.
...
{2, [Second]} = graphics_mesh2:local_vertices(Mesh, {1, -1}).
```
""".
...
```erlang
{ok, Mesh} = graphics_mesh2:with_vertices(Vertices, static, keep_copy),
{1, {{0.0, 0.0}, ?COLOR_RED, 0.0, 0.0}} = graphics_mesh2:local_vertex(Mesh, 0),
{3, {{0.0, 1.0}, ?COLOR_BLUE, 0.0, 1.0}} = graphics_mesh2:local_vertex(Mesh, -1).
```
