...
matrix itself is a `graphics:matrix3()` value; see the `graphics_matrix3` module for
linear algebra.

```erlang
M = graphics_transform2:translation({10.0, 20.0}).
{11.0, 22.0} = graphics_transform2:transform_point(M, {1.0, 2.0}).
```
...
```erlang
M1 = graphics_transform2:translate(graphics_matrix3:identity(), {10.0, 0.0}),
M2 = graphics_transform2:rotate(M1, math:pi() / 2.0).
```
...
Rotation is counter-clockwise around the origin, matching `graphics_vector2:rotate/2`.
...
```erlang
{11.0, 22.0} = graphics_transform2:transform_point(
...
radians. The rotation is counter-clockwise, matching `graphics_vector2:rotate/2`.
""".
...
```erlang
{2.0, 6.0} = graphics_transform2:transform_point(
...
translate(Matrix, Vector) ->
    graphics_matrix3:multiply(Matrix, translation(Vector)).
...
rotate(Matrix, Angle) ->
    graphics_matrix3:multiply(Matrix, rotation(Angle)).
...
scale(Matrix, Vector) ->
    graphics_matrix3:multiply(Matrix, scale(Vector)).
...
compose(Position, Rotation, Scale) ->
    compose(graphics_vector2:zero(), Position, Rotation, Scale).
...
compose(Origin, Position, Rotation, Scale) ->
    graphics_matrix3:multiply(
        translation(Position),
        graphics_matrix3:multiply(
            rotation(Rotation),
            graphics_matrix3:multiply(
                scale(Scale),
                translation(graphics_vector2:negate(Origin))
            )
        )
    ).
...
decompose({M11, M21, _, M12, M22, _, M13, M23, _}) ->
    Translation = {M13, M23},
    ScaleX = graphics_vector2:length({M11, M21}),
    ScaleY = graphics_vector2:length({M12, M22}),
    Det = M11 * M22 - M12 * M21,
    ScaleYSigned = if
        Det < 0.0 ->
            -ScaleY;
...
This is the same computation as `graphics_matrix3:multiply_vector/2`.
""".
-spec transform_point(graphics:matrix3(), graphics:vector2()) ->
    graphics:vector2().
transform_point(Matrix, Point) ->
    graphics_matrix3:multiply_vector(Matrix, Point).
...
transform_box(Matrix, Box) ->
    graphics_box2:from_points([transform_point(Matrix, Corner) || Corner <- graphics_box2:corners(Box)]).
