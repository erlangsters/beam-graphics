...
```erlang
M = view2:orthographic(0.0, 800.0, 0.0, 600.0).
{-1.0, -1.0} = matrix3:multiply_vector(M, {0.0, 0.0}).
{1.0, 1.0} = matrix3:multiply_vector(M, {800.0, 600.0}).
```
...
```erlang
M = view2:orthographic({{0.0, 0.0}, {800.0, 600.0}}).
{-1.0, -1.0} = matrix3:multiply_vector(M, {0.0, 0.0}).
```
""".
...
```erlang
{-1.0, 1.0} = matrix3:multiply_vector(
    view2:orthographic(0.0, 800.0, 600.0, 0.0),
    {0.0, 0.0}
).
```
""".
