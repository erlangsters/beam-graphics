...
```erlang
{0.0, 0.0} = box2:min(Box).
{10.0, 20.0} = box2:max(Box).
```
...
```erlang
{{1.0, 2.0}, {5.0, 6.0}} = box2:from_points([{1.0, 2.0}, {5.0, 6.0}]).
{{-5.0, -10.0}, {5.0, 10.0}} = box2:from_center_size({0.0, 0.0}, {10.0, 20.0}).
```
...
```erlang
true = box2:contains({{0.0, 0.0}, {10.0, 10.0}}, {5.0, 5.0}).
true = box2:intersects({{0.0, 0.0}, {2.0, 2.0}}, {{1.0, 1.0}, {3.0, 3.0}}).
```

To transform a 2D box with a 3x3 matrix, see the `transform2:transform_box/2`
...
from_center_size(Center, Size) ->
    Half = vector2:multiply(vector2:abs(Size), 0.5),
    {vector2:subtract(Center, Half), vector2:add(Center, Half)}.
...
center({Min, Max}) ->
    vector2:multiply(vector2:add(Min, Max), 0.5).
...
intersection({Min1, Max1}, {Min2, Max2}) ->
    Min = vector2:max(Min1, Min2),
    Max = vector2:min(Max1, Max2),
    {MinX, MinY} = Min,
    {MaxX, MaxY} = Max,
...
union({Min1, Max1}, {Min2, Max2}) ->
    {vector2:min(Min1, Min2), vector2:max(Max1, Max2)}.
...
translate({Min, Max}, Vector) ->
    {vector2:add(Min, Vector), vector2:add(Max, Vector)}.
...
is_equal_to({Min1, Max1}, {Min2, Max2}) ->
    vector2:is_equal_to(Min1, Min2) andalso vector2:is_equal_to(Max1, Max2).
...
is_equal_to({Min1, Max1}, {Min2, Max2}, Epsilon) ->
    vector2:is_equal_to(Min1, Min2, Epsilon)
        andalso vector2:is_equal_to(Max1, Max2, Epsilon).
