...
```erlang
{ok, Frame} = graphics_frame:with_size({640, 480}),
ok = graphics_frame:clear(Frame, ?COLOR_BLACK),
ok = graphics_frame:draw_mesh2(Frame, Mesh, triangles, 3),
...
`draw_mesh2/4` and of `graphics_shape2`. The same holds for 3D meshes and `graphics_shape3`.
...
2D view or camera matrix is 3x3; embed it with `graphics_matrix3:to_matrix4/1`.
...
framebuffer id. The color texture id is `graphics_texture:gl_object(graphics_frame:texture(Frame))`.
Custom shaders are not a draw argument. Build a `graphics_program`, take
`graphics_program:gl_object/1`, and issue GL calls through
...
-opaque object() :: {
    ...
    Program :: graphics_program:object(),
    Viewport :: viewport(),
...
with_size({Width, Height}) when Width > 0, Height > 0 ->
    case graphics_texture:with_color(?COLOR_BLACK, {Width, Height}) of
        out_of_memory ->
            out_of_memory;
        {ok, Texture} ->
            ...
            case acquire_frame(Width, Height, GlTexture) of
                {error, out_of_memory} ->
                    ...
                    out_of_memory;
                {ok, ResourceId} ->
                    case graphics_default_program:new() of
                        {ok, Program} ->
                            Frame = {
                                ResourceId,
                                {Width, Height},
...
resize(
    ...
    case graphics_texture:resize(Texture, Size) of
        out_of_memory ->
            out_of_memory;
        {ok, NewTexture} ->
            GlTexture = graphics_texture:gl_object(NewTexture),
...
It's equivalent to `draw_mesh2(Frame, Mesh, PrimitiveType, VertexCount, Texture, graphics_matrix3:identity())`.
""".
-spec draw_mesh2(
    object(),
    graphics:mesh2(),
...
draw_mesh2(
    ...
    ok = graphics_program:set_uniform(Program, "uModel", graphics_matrix3:to_matrix4(Matrix3)),
    ok = graphics_program:set_uniform(Program, "uView", ViewMatrix),
    ok = graphics_program:set_uniform(Program, "uProjection", ProjectionMatrix),
    ok = graphics_program:set_uniform(Program, "uUseTexture", Texture =/= no_texture),
    ok = frame_draw(
        Framebuffer,
        Viewport,
        graphics_program:gl_object(Program),
        BlendMode,
        DepthTest,
...
It's equivalent to `draw_mesh3(Frame, Mesh, PrimitiveType, VertexCount, Texture, graphics_matrix4:identity())`.
""".
-spec draw_mesh3(
    object(),
    graphics:mesh3(),
...
draw_mesh3(
    ...
    ok = graphics_program:set_uniform(Program, "uModel", Matrix),
    ok = graphics_program:set_uniform(Program, "uView", ViewMatrix),
    ok = graphics_program:set_uniform(Program, "uProjection", ProjectionMatrix),
    ok = graphics_program:set_uniform(Program, "uUseTexture", Texture =/= no_texture),
    ok = frame_draw(
