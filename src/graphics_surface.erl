...
arguments of `draw_mesh2/4` and of `graphics_shape2`. The same holds for 3D meshes and
`graphics_shape3`. The stock program modulates vertex color by the bound texture. That
...
a `graphics_program`, take `graphics_program:gl_object/1`, and issue GL calls through
`gl_commands/2`.
""".
...
It's equivalent to `draw_mesh2(Surface, Mesh, PrimitiveType, VertexCount, Texture, graphics_matrix3:identity())`.
""".
-spec draw_mesh2(
    object(),
    graphics:mesh2(),
...
draw_mesh2(
    ...
    Request = {
        ...
        VertexCount,
        Texture,
        graphics_matrix3:to_matrix4(Matrix3),
        BlendMode,
        DepthTest
    },
...
It's equivalent to `draw_mesh3(Surface, Mesh, PrimitiveType, VertexCount, Texture, graphics_matrix4:identity())`.
""".
-spec draw_mesh3(
    object(),
    graphics:mesh3(),
...
use this path with `graphics_program:gl_object/1`.
...
handle_request(
    ...
    ok = gl:program_uniform_matrix(
        f, GlProgram, ModelLocation, graphics_matrix4:columns(ModelMatrix)
    ),
    ok = gl:program_uniform_matrix(
        f, GlProgram, ViewLocation, graphics_matrix4:columns(ViewMatrix)
    ),
    ok = gl:program_uniform_matrix(
        f, GlProgram, ProjectionLocation, graphics_matrix4:columns(ProjectionMatrix)
    ),
