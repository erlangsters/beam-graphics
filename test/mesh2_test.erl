...
mesh2_test() ->
    ...
    false = mesh2:has_local_copy(Mesh),
...
mesh2_with_vertices_test() ->
    ...
    true = mesh2:has_local_copy(Mesh3),

    {ok, Mesh4} = mesh2:with_vertices([], static, keep_copy),
    0 = mesh2:vertex_count(Mesh4),
    [] = mesh2:local_vertices(Mesh4),
    true = mesh2:has_local_copy(Mesh4),
...
mesh2_set_vertices_test() ->
    ...
    Buffer = mesh2:gl_object(Mesh3),

    {ok, MeshKeep} = mesh2:with_vertices(?VERTICES, dynamic, keep_copy),
    KeepBuffer = mesh2:gl_object(MeshKeep),
    {ok, MeshKeep2} = mesh2:set_vertices(MeshKeep, [?VERTEX_3]),
    1 = mesh2:vertex_count(MeshKeep2),
    dynamic = mesh2:usage_hint(MeshKeep2),
    [?VERTEX_3] = mesh2:local_vertices(MeshKeep2),
    [?VERTEX_3] = mesh2:remote_vertices(MeshKeep2),
    true = mesh2:has_local_copy(MeshKeep2),
    KeepBuffer = mesh2:gl_object(MeshKeep2),

    {ok, MeshKeep3} = mesh2:set_vertices(
        MeshKeep2,
        [?VERTEX_3, ?VERTEX_1, ?VERTEX_2],
        stream
    ),
    3 = mesh2:vertex_count(MeshKeep3),
    stream = mesh2:usage_hint(MeshKeep3),
    [?VERTEX_3, ?VERTEX_1, ?VERTEX_2] = mesh2:local_vertices(MeshKeep3),
    [?VERTEX_3, ?VERTEX_1, ?VERTEX_2] = mesh2:remote_vertices(MeshKeep3),
    KeepBuffer = mesh2:gl_object(MeshKeep3),

    ok = mesh2:destroy(Mesh3),
    ok = mesh2:destroy(MeshKeep3),
...
mesh2_usage_hint_test() ->
    ...
    {ok, Mesh4} = mesh2:set_vertices(Mesh3, [?VERTEX_1]),
    static = mesh2:usage_hint(Mesh4),
...
mesh2_local_vertices_test() ->
    ...
    {ok, Mesh1} = mesh2:with_vertices(?VERTICES, static, no_copy),
    undefined = mesh2:local_vertices(Mesh1),
    undefined = mesh2:local_vertex(Mesh1, 1),
    undefined = mesh2:local_vertices(Mesh1, {1, -1}),

    {ok, Mesh2} = mesh2:with_vertices(?VERTICES, static, keep_copy),
...
mesh2_update_vertices_test() ->
    ...
    {ok, MeshNoCopy2} =
        mesh2:update_vertices(MeshNoCopy1, [?VERTEX_1, ?VERTEX_3], 0),
    ...
    {ok, MeshNoCopy3} =
        mesh2:update_vertices(MeshNoCopy2, [?VERTEX_2, ?VERTEX_1], 1),
    ...
    {ok, MeshNoCopy4} = mesh2:update_vertices(MeshNoCopy3, [?VERTEX_3], 2),
    ...
    out_of_range =
        mesh2:update_vertices(MeshNoCopy4, [?VERTEX_3, ?VERTEX_1], 2),

    {ok, MeshNoCopy5} = mesh2:update_vertex(MeshNoCopy4, ?VERTEX_3, 0),
    undefined = mesh2:local_vertices(MeshNoCopy5),
    [?VERTEX_3, ?VERTEX_2, ?VERTEX_3] = mesh2:remote_vertices(MeshNoCopy5),
    {ok, MeshNoCopy6} = mesh2:update_vertex(MeshNoCopy5, ?VERTEX_1, -1),
    undefined = mesh2:local_vertices(MeshNoCopy6),
    [?VERTEX_3, ?VERTEX_2, ?VERTEX_1] = mesh2:remote_vertices(MeshNoCopy6),

    out_of_range = mesh2:update_vertex(MeshNoCopy6, ?VERTEX_3, 3),

    {ok, MeshNoCopy7} = mesh2:update_vertices(MeshNoCopy6, []),
    [?VERTEX_3, ?VERTEX_2, ?VERTEX_1] = mesh2:remote_vertices(MeshNoCopy7),

    ok = mesh2:destroy(MeshNoCopy7),
    ...
    {ok, MeshKeepCopy2} =
        mesh2:update_vertices(MeshKeepCopy1, [?VERTEX_1, ?VERTEX_3], 0),
    ...
    {ok, MeshKeepCopy3} =
        mesh2:update_vertices(MeshKeepCopy2, [?VERTEX_2, ?VERTEX_1], 1),
    ...
    {ok, MeshKeepCopy4} = mesh2:update_vertices(MeshKeepCopy3, [?VERTEX_3], 2),
    ...
    out_of_range =
        mesh2:update_vertices(MeshKeepCopy4, [?VERTEX_3, ?VERTEX_1], 2),

    {ok, MeshKeepCopy5} = mesh2:update_vertex(MeshKeepCopy4, ?VERTEX_3, 0),
    ...
    {ok, MeshKeepCopy6} = mesh2:update_vertex(MeshKeepCopy5, ?VERTEX_1, -1),
    ...
    out_of_range = mesh2:update_vertex(MeshKeepCopy6, ?VERTEX_3, 3),

    ok = mesh2:destroy(MeshKeepCopy6),
...
mesh2_local_copy_test() ->
    ...
    {ok, Mesh0} = mesh2:with_vertices(?VERTICES),

    false = mesh2:has_local_copy(Mesh0),
    undefined = mesh2:local_vertices(Mesh0),

    {ok, Mesh1} = mesh2:keep_local_copy(Mesh0),
    true = mesh2:has_local_copy(Mesh1),
    ?VERTICES = mesh2:local_vertices(Mesh1),
    ?VERTICES = mesh2:remote_vertices(Mesh1),

    already_local_copy = mesh2:keep_local_copy(Mesh1),

    {ok, Mesh2} = mesh2:release_local_copy(Mesh1),
    false = mesh2:has_local_copy(Mesh2),
    undefined = mesh2:local_vertices(Mesh2),
    ?VERTICES = mesh2:remote_vertices(Mesh2),

    no_local_copy = mesh2:release_local_copy(Mesh2),
