...
either a pbuffer (`graphics_surface:with_size/2`) or a window drawable
(`graphics_surface:with_window/3`).
...
contextless: EGL owns the context, which is what `graphics_surface` already does.
...
For offscreen work, keep `graphics_surface:with_size/2` and `graphics_surface:image/1`, or draw
into a `graphics_frame` and sample `graphics_frame:texture/1`. See [Texturing](texturing.md).
