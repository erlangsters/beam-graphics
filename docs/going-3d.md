...
`graphics_vector3:multiply/2` is scalar multiplication. The 3D cross product is a
vector. `rotate/3` is Rodrigues rotation around an axis. There is no polar
`to_angle` / `from_angle`; that conversion is 2D-only.

See the `graphics_vector3` module.

### The 4x4 matrix

A 4x4 matrix is a flat 16-float tuple in column-major order. It is the 3D
transform type. Prefer `graphics_transform3` for translation, rotation, and scale.

```erlang
I = graphics_matrix4:identity().
{11.0, 22.0, 33.0} = graphics_matrix4:multiply_vector(
    graphics_transform3:translation({10.0, 20.0, 30.0}),
    {1.0, 2.0, 3.0}
).
```

`graphics_matrix4:multiply/2` is the matrix product. `graphics_matrix4:scale/2` is scalar
multiplication. `graphics_matrix4:multiply_vector/2` treats the vector as a point
(homogeneous `w = 1.0`). Extract the 2D affine block with `graphics_matrix4:to_matrix3/1`.

See the `graphics_matrix4` module.
...
```erlang
{0.0, 0.0, 1.0, 1.0} = graphics_color:rgb(0.0, 0.0, 1.0).
...
Textured 3D geometry is built with `graphics_shape3:with_mesh/4`; see
...
{5.0, 10.0, 15.0} = graphics_box3:center(Box).
true = graphics_box3:contains(Box, {5.0, 5.0, 5.0}).
```
...
on `graphics_transform3:transform_box/2`.

See the `graphics_box3` module.

### The 3D transform

There is no transform type. `graphics_transform3` constructs and applies 4x4 matrices.
...
{11.0, 22.0, 33.0} = graphics_transform3:transform_point(
    graphics_transform3:translation({10.0, 20.0, 30.0}),
    {1.0, 2.0, 3.0}
).
```
...
See the `graphics_transform3` module.
...
`look_at` lives on `graphics_camera3`, not on `graphics_view3`.

There is no view wrapper type. `graphics_view3` constructs orthographic, perspective,
and frustum matrices. The convention is OpenGL right-handed, clip Z in
`[-1, 1]`. Y increases upward when Bottom is less than Top.

```erlang
P = graphics_view3:perspective(math:pi() / 4.0, 640.0 / 480.0, 0.1, 100.0).
O = graphics_view3:orthographic(-1.0, 1.0, -1.0, 1.0, 0.1, 100.0).
```
...
V = graphics_camera3:view_matrix(Camera).
```
...
See the `graphics_view3` and `graphics_camera3` modules.
...
`graphics_view3:perspective/4` or `graphics_view3:orthographic/6`. Keep the depth test enabled
...
See the `graphics_shape3` module.
...
```erlang
Shape = graphics_shape3:with_mesh(Mesh, triangles, 3).
```

`with_mesh` does not allocate. The vertex count is the draw count. It is not
required to equal `graphics_mesh3:vertex_count/1`.
...
```erlang
{ok, Cube} = graphics_shape3:cube({0.0, 0.0, 0.0}, {1.0, 1.0, 1.0}, ?COLOR_RED),
Moved = graphics_shape3:set_matrix(Cube, graphics_transform3:translation({0.0, 0.5, 0.0})),
ok = graphics_surface:draw_shape3(Surface, Moved).
```
...
Rotated = graphics_shape3:set_matrix(Cube, M).
```

`draw_mesh3/6` takes the 4x4 model matrix directly. Combinators post-multiply:

```erlang
M1 = graphics_transform3:translate(graphics_matrix4:identity(), {0.0, 0.5, 0.0}),
M2 = graphics_transform3:rotate(M1, math:pi() / 4.0, {0.0, 1.0, 0.0}),
ok = graphics_surface:draw_mesh3(Surface, Mesh, triangles, 3, no_texture, M2).
```
...
3D orthographic) from `graphics_view3`, and a look-at from `graphics_camera3`. Both matrices
are already 4x4.
...
See the `graphics_view3`, `graphics_camera3`, and `graphics_surface` modules.
