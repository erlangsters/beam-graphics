...
```erlang
V = {3.0, 4.0}.
3.0 = graphics_vector2:x(V).
4.0 = graphics_vector2:y(V).
5.0 = graphics_vector2:length(V).
{0.6, 0.8} = graphics_vector2:normalize(V).
```

`graphics_vector2:multiply/2` is scalar multiplication. The 2D cross product is a
scalar. Rotation is counter-clockwise around the origin. Polar conversion
(`to_angle/1`, `from_angle/1`) is 2D-only.

See the `graphics_vector2` module.

### The 3x3 matrix

A 3x3 matrix is a flat 9-float tuple in column-major order. It is the 2D
transform type. Prefer `graphics_transform2` for translation, rotation, and scale.

```erlang
I = graphics_matrix3:identity().
{11.0, 22.0} = graphics_matrix3:multiply_vector(
    graphics_transform2:translation({10.0, 20.0}),
    {1.0, 2.0}
).
```

`graphics_matrix3:multiply/2` is the matrix product. `graphics_matrix3:scale/2` is scalar
multiplication. `graphics_matrix3:multiply_vector/2` treats the vector as a point
(homogeneous `w = 1.0`). Embed a 3x3 as a 4x4 with `graphics_matrix3:to_matrix4/1`
when a surface or frame needs a view or projection matrix.

See the `graphics_matrix3` module.
...
```erlang
{1.0, 0.0, 0.0, 1.0} = graphics_color:rgb(1.0, 0.0, 0.0).
?COLOR_RED = {1.0, 0.0, 0.0, 1.0}.
```

See the `graphics_color` module.
...
{5.0, 10.0} = graphics_box2:center(Box).
true = graphics_box2:contains(Box, {5.0, 5.0}).
```
...
`graphics_transform2:transform_box/2`.

See the `graphics_box2` module.

### The 2D transform

There is no transform type. `graphics_transform2` constructs and applies 3x3 matrices.
...
```erlang
M = graphics_transform2:compose({100.0, 50.0}, math:pi() / 4.0, {2.0, 2.0}).
{110.0, 70.0} = graphics_transform2:transform_point(
    graphics_transform2:translation({10.0, 20.0}),
    {100.0, 50.0}
).
```
...
See the `graphics_transform2` module.
...
There is no view wrapper type. `graphics_view2:orthographic/4` maps a rectangle of
view space to clip space. Y increases upward when Bottom is less than Top.

```erlang
P = graphics_view2:orthographic(0.0, 640.0, 0.0, 480.0).
P = graphics_view2:orthographic({{0.0, 0.0}, {640.0, 480.0}}).
```
...
V = graphics_camera2:view_matrix(Camera).
```
...
See the `graphics_view2` and `graphics_camera2` modules.
...
A **surface** presents. A pbuffer is created with `graphics_surface:with_size/2`. A
window is created with `graphics_surface:with_window/3`; see
...
On a window, call `graphics_surface:display/1` instead of (or before) reading pixels.
After `display/1`, the window back buffer is undefined.

See the `graphics_surface`, `graphics_frame`, and `graphics_context` modules.
...
ok = graphics_surface:draw_mesh2(Surface, Mesh, triangles, 3),
...
```erlang
Shape = graphics_shape2:with_mesh(Mesh, triangles, 3).
```

`with_mesh` does not allocate. The vertex count is the draw count. It is not
required to equal `graphics_mesh2:vertex_count/1`.
...
```erlang
M = graphics_transform2:compose(
    {50.0, 25.0},
    {320.0, 240.0},
    math:pi() / 8.0,
    {1.0, 1.0}
),
Rotated = graphics_shape2:set_matrix(Rect, M).
```

`draw_mesh2/6` takes the 3x3 model matrix directly. Combinators post-multiply:

```erlang
M1 = graphics_transform2:translate(graphics_matrix3:identity(), {320.0, 240.0}),
M2 = graphics_transform2:rotate(M1, math:pi() / 4.0),
ok = graphics_surface:draw_mesh2(Surface, Mesh, triangles, 3, no_texture, M2).
```
...
To choose a visible rectangle, set the projection from `graphics_view2`. To move the
observer, set the view from `graphics_camera2`. Both matrices are 3x3. A surface and
...
Camera = graphics_camera2:from_center({320.0, 240.0}, 0.0, 1.0),
...
Swap Bottom and Top in `orthographic/4` to flip Y. A well-formed `graphics_box2`
...
See the `graphics_view2`, `graphics_camera2`, and `graphics_surface` modules.
