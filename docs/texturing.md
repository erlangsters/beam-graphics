...
Dispose it with `texture:destroy/1`. A running graphics context is required.
...
```erlang
{ok, Texture} = texture:set_minification_filter(Texture, nearest),
{ok, Texture} = texture:set_magnification_filter(Texture, nearest),
{ok, Texture} = texture:set_wrap_mode(Texture, horizontal, repeat),
{ok, Texture} = texture:set_wrap_mode(Texture, vertical, repeat).
```
...
On a shape, several meshes share one texture:

```erlang
Shape = shape2:with_mesh(Mesh, triangle_fan, 4, Texture),
ok = surface:draw_shape2(Surface, Shape).
```

`shape2:set_texture/2` returns a new shape. It does not destroy the previous
texture.
...
The same calls exist on `frame` and for `mesh3` / `shape3`. There is no
`sprite3`; textured 3D geometry stays on `shape3:with_mesh/4`.
...
A sprite is a textured 2D rectangle. There is no sprite type. `sprite`
...
that reads `texture:size/1`.

See the `sprite` module.
...
ok = frame:draw_shape2(Frame, Shape),
Texture = frame:texture(Frame),
ok = surface:draw_mesh2(Surface, Quad, triangle_fan, 4, Texture).
```

The frame owns that texture. `frame:destroy/1` destroys it. Do not call
`texture:destroy/1` on it while the frame is alive. Using the texture after
...
Read CPU pixels from a frame with `texture:remote_image(frame:texture(Frame))`.
Frame has no CPU image function of its own.

See the `frame` module.
...
`texture:with_image/1`. Save a surface with `surface:image/1`.
...
The codec does not Y-flip. That matches `texture`.
